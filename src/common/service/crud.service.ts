...
export interface ICrudService<T extends ObjectLiteral> {
    ...
    update(params: { id: number; dto: DeepPartial<T> }): Promise<T>;
    ...
    findBy(params: { filters: FindOptionsWhere<T> | FindOptionsWhere<T>[]; relations?: FindOptionsRelations<T>; page: number; size: number }): Promise<PaginationModel<T>>;
