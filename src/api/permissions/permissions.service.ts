...
export class PermissionsService {
    ...
    async create(dto: DeepPartial<Permission>): Promise<Permission> {
        const existing = await this.permissionRepo.findOne({ where: { path: dto.path, method: dto.method } });
    ...
    async update({ id, dto }: { id: number; dto: DeepPartial<Permission> }): Promise<Permission> {
        const permission = await this.findOneBy({ filters: { id } });
        Object.assign(permission, dto);
        return this.permissionRepo.save(permission);
    }

    async findOneBy(params: { filters: FindOptionsWhere<Permission> | FindOptionsWhere<Permission>[]; relations?: FindOptionsRelations<Permission> }): Promise<Permission> {
        const permission = await this.permissionRepo.findOne({ where: params.filters, relations: params.relations });
        if (!permission) throw new NotFoundException('Permission not found');
        return permission;
    }

    async findBy(params: { filters: FindOptionsWhere<Permission> | FindOptionsWhere<Permission>[]; relations?: FindOptionsRelations<Permission>; page: number; size: number }): Promise<PaginationModel<Permission>> {
